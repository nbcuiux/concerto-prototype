...
function filterData(item, filter) {
    switch (filter.id) {
        ...
        case 'season':
            return item.season ? parseInt(item.season) === parseInt(filter.value) : false;

        default:
            return item[filter.id] != undefined ? item[filter.id].toString().toLowerCase() === filter.value.toString().toLowerCase() : false;
    }
}
...
function renderContentRow(data) {
    ...
    //Thumbnail
    var thumbnailCell = $('<td></td>').addClass('library__cell');
    if (data.thumbnail) {
        ...
        var thumbnail = $('<div></div>').append('<i class="fa fa-file-image-o" aria-hidden="true"></i>');
        thumbnailCell.addClass('library__cell-no-thumbnail').append(thumbnail);
    }
    ...
    var statusCell = $('<td></td>').addClass('library__cell'),
        ...
        status = $('<div></div>')
                    ...
                    .addClass(data.status.toLowerCase() === 'draft' ? 'library__cell-value--draft' : '')
                    .addClass(data.status.toLowerCase() === 'scheduled' ? 'library__cell-value--scheduled' : '')
                    .text(data.status);
    ...
    var rowEdit = $('<div></div>')
                    ...
                    .click(function(e) {
                        //window.location.href = $(e.target).parents('.library__row--content').attr('data-target');
                        var elem = $(e.target).parents('.library__row--content');
...
function setRevisionsSorting(sorting, renderer) {
    collectionStore.setSorting(sorting);
    renderData($('#libraryBody'), collectionStore, renderer);
    //renderContent($('#contentLibrary'), store, renderContentHeaderRow, renderContentRow);
}

//UI Actions
function handleRevisionContentSorting(e) {
    var id = e.target.dataset.sortId,
        direction = $(e.target).hasClass('js-ascending') ? 'descending' : 'ascending';
    ...
    setRevisionsSorting({id: id, direction: direction}, renderRevisionRow );
    $('.library__header').removeClass('is-open');
    $('#sortContentDropdown').removeClass('is-pressed');
}


function renderRevisionHeaderRow(store) {
    var row = $('<tr></tr>').addClass('library__row library__row--header'),
        //thumbnailCell = $('<div></div>').addClass('library__cell library__cell--header').text('Thumbnail'),
        favCell = $('<th></th>').addClass('library__cell library__cell--header')
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        .find('.library__cell-header-text')
                        .text('')
                        .parent(),
        vid = $('<th></th>').addClass('library__cell library__cell--header library__cell--sortable')
                        .attr('data-sort-id', 'title')
                        .click(handleRevisionContentSorting)
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        .find('.library__cell-header-text')
                        .text('VID')
                        .parent(),
        ...
        statusCell = $('<th></th>').addClass('library__cell library__cell--header library__cell--sortable')
                        ...
                        .text('Status')
                        .parent(),
        versionCell = $('<th></th>').addClass('library__cell library__cell--header library__cell--sortable')
                        .attr('data-sort-id', 'series')
                        .click(handleRevisionContentSorting)
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        .find('.library__cell-header-text')
                        .text('Version')
                        .parent(),
        historyCell = $('<th></th>').addClass('library__cell library__cell--header library__cell--sortable')
                        .attr('data-sort-id', 'series')
                        .click(handleRevisionContentSorting)
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        .find('.library__cell-header-text')
        ...
        updatedCell = $('<div></div>').addClass('library__cell library__cell--header library__cell--sortable')
                        ...
                        .text('Updated')
                        .parent();
        editCell = $('<div></div>').addClass('library__cell library__cell--header library__cell--sortable')
                        .attr('data-sort-id', 'updateDate')
                        .click(handleRevisionContentSorting)
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        .find('.library__cell-header-text')
    ...
    row.append(favCell, vid, titleCell, typeCell, statusCell, versionCell, historyCell, updatedCell, editCell);
    row.find('.library__cell--header[data-sort-id="' + store.sorting.id + '"]').addClass('js-' + store.sorting.direction);

    return row;
}
function renderRevisionRow(data) {
    var row = $('<tr></tr>')
                .addClass('library__row library__row--content is-folded')
                .attr('data-target', data.target)
                .attr('data-id', data.id);
    ...
    //Fav
    var favCell = $('<td></td>').addClass('library__cell').addClass('library__cell--fav'),
        fav = $('<div class="c-Header-favorite"><input type="checkbox" class="favorite" id="toDashboard"><label for="toDashboard">Add to favorites</label></div>');
    row.append(favCell.append(fav));


    //vid
    var vidCell = $('<td></td>').addClass('library__cell'),
        vid = $('<div></div>')
                .addClass('library__cell-value library__cell-value--title')
                .text(data.vid)
                .attr('vid', data.vid);
    row.append(vidCell.append(vid));
    ...
    //Version
    var versionCell = $('<td></td>').addClass('library__cell'),
        version = $('<div></div>')
                    .addClass('library__cell-value')
                    .text(data.version)
                    .attr('version', data.version);
    row.append(versionCell.append(version));

    //history
    var historyCell = $('<td></td>').addClass('library__cell'),
        history = $('<div></div>')
                    .addClass('library__cell-value')
                    .html(data.history)
                    .attr('history', data.history);
    row.append(historyCell.append(history));
    ...
    var updateCell = $('<div></div>').addClass('library__cell'),
        updateValue = $('<div></div>').addClass('library__cell-value'),
        updateDate = $('<div></div>').addClass('library__value-date')
                                    .attr("data-timestamp", data.updateDate.valueOf())
                                     .text(data.updateDate.toLocaleDateString('en-US') + ' ' + data.updateDate.toLocaleTimeString('en-US', timeOptions));
        //updateName = $('<div></div>').addClass('library__value-name').text('by ' + data.updateName);
    updateValue.append(updateDate);
    row.append(updateCell.append(updateValue));
...
function setRevisionsSortingPage(sorting, renderer) {
    collectionStore.setSorting(sorting);
    renderData($('#libraryBody'), collectionStore, renderer);
    //renderContent($('#contentLibrary'), store, renderContentHeaderRow, renderContentRow);
}
...
//UI Actions
function handleRevisionContentSortingPage(e) {
    var id = e.target.dataset.sortId,
        direction = $(e.target).hasClass('js-ascending') ? 'descending' : 'ascending';
    ...
    setRevisionsSortingPage({id: id, direction: direction}, renderRevisionRowPage );
    $('.library__header').removeClass('is-open');
    $('#sortContentDropdown').removeClass('is-pressed');
}


function renderRevisionHeaderRowPage(store) {
    var row = $('<tr></tr>').addClass('library__row library__row--header'),
        //thumbnailCell = $('<div></div>').addClass('library__cell library__cell--header').text('Thumbnail'),
        favCell = $('<th></th>').addClass('library__cell library__cell--header')
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        ...
                        .parent(),
        vid = $('<th></th>').addClass('library__cell library__cell--header library__cell--sortable')
                        .attr('data-sort-id', 'vid')
                        .click(handleRevisionContentSortingPage)
                        .append($('<span></span>').addClass('library__cell-header-text'))
                        .find('.library__cell-header-text')
                        .text('VID')
                        .parent(),
    ...
    row.append(favCell, vid, statusCell, versionCell, historyCell, updatedCell, editCell);
    row.find('.library__cell--header[data-sort-id="' + store.sorting.id + '"]').addClass('js-' + store.sorting.direction);

    return row;
}

function renderRevisionRowPage(data) {
    var row = $('<tr></tr>')
                .addClass('library__row library__row--content is-folded')
                .attr('data-target', data.target)
                .attr('data-id', data.id);
    ...
    var versionCell = $('<td></td>').addClass('library__cell'),
        version = $('<div></div>')
                    .addClass('library__cell-value library__cell-version')
                    .text(data.version)
                    .attr('version', data.version);
    row.append(versionCell.append(version));
